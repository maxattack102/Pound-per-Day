...
{   
    ...
    if ($result->num_rows == 1) 
    {
      ...
      $data = array("username"=>$username, "fname"=>$fname, "lname"=>$lname, "age"=>$age, "weight"=>$weight, "height_ft"=>$height_ft, "height_in"=>$height_in, "gender"=>$gender, "loss"=>$loss, "activity"=>$activity);
...
function show($username) //, &$output
{
    ...
    $data = array("username"=>$username, "fname"=>$fname, "lname"=>$lname, "age"=>$age, "weight"=>$weight);
    //$all_info = array('data'=>$data);
    echo "\n\n\t***Show Profile***\n\n";
    return json_encode($data);
    //echo $output;
...
function caloriecalculator(){
	global $db;
	$s = "select * from foodsyste1 where user = '$user'";
